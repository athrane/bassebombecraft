...
public class JsonBlock {
	...
	public JsonBlock(BlockDirective directive, String name) {
		...
		IBlockState state = directive.getState();		
		this.id = GameRegistry.findUniqueIdentifierFor(state.getBlock());				
		stateProperties = state.getProperties();		
						
		Block block = directive.getBlock();
		...
		for(IProperty key : keys) {
			...
			// inspect key
			Class keyValue = key.getValueClass();
			...
			// inspect value
			Object value = stateProperties.get(key);
