...
public class AttackNearestMobCommand implements MobCommand {
	...
	public boolean shouldExecute(EntityPlayer commander, EntityCreature entity) {
		...
		List<EntityMob> mobList = entity.world.getEntitiesWithinAABB(EntityMob.class, aabb, discardMembersFilter);

		// exit if no mobs where found
		if (mobList.isEmpty())
			return false;

		// sort mobs
		entityDistanceSorter.setEntity(entity);
		Collections.sort(mobList, entityDistanceSorter);

		// get target
		EntityMob target = mobList.get(FIRST_INDEX);
